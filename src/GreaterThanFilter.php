<?php

namespace Bermuda\Filter;

/**
 * GreaterThanFilter
 *
 * Accepts an element if its value (cast to a float) is greater than a specified threshold.
 */
final class GreaterThanFilter extends AbstractFilter implements FilterInterface
{
    /**
     * @var float The threshold value.
     */
    private float $threshold;

    /**
     * Constructor.
     *
     * @param float $threshold The threshold the element’s numeric value must exceed.
     * @param iterable $iterable The data source to be filtered.
     */
    public function __construct(float $threshold, iterable $iterable = [])
    {
        $this->threshold = $threshold;
        parent::__construct($iterable);
    }

    /**
     * Returns a new instance with an updated threshold.
     *
     * @param float $threshold The new threshold.
     * @return self A new GreaterThanFilter instance with the updated threshold.
     */
    public function withThreshold(float $threshold): self
    {
        $copy = clone $this;
        $copy->threshold = $threshold;
        return $copy;
    }

    /**
     * Determines whether the given element should be accepted.
     *
     * The element is accepted if its value (cast to float) is greater than the threshold.
     *
     * @param int|string $key The key associated with the element.
     * @param mixed $value The element to evaluate.
     * @return bool True if (float)$value > $threshold; otherwise, false.
     */
    public function accept(int|string $key, mixed $value): bool
    {
        return (float)$value > $this->threshold;
    }
}